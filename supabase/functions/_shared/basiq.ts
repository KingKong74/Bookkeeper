...
export function normaliseBasiqTxn(bt: any, bankAccountId: string, orgId: string): object {
  return {
    org_id:      orgId,
    account_id:  bankAccountId,    // your internal bank_accounts.id
    date:        bt.postDate?.slice(0, 10) ?? bt.transactionDate?.slice(0, 10),
    description: bt.description ?? bt.merchant?.businessName ?? '',
    amount:      parseFloat(bt.amount ?? '0'),
    currency:    bt.currency ?? 'AUD',
    // Hash for deduplication: use Basiq's own ID
