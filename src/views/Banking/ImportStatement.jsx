...
// ── Parse a single file, return { filename, transactions, summary, error } ────
...
export function ImportStatement({ onNavigate }) {
  ...
  const fileRowRefs = useRef({});   // keyed by fi → <tbody section> DOM node

  // ── Flat merged transaction list with source tracking ─────────────────────
  ...
  const autoCatMap = useMemo(() => {
    if (!allTransactions.length || !(rules||[]).length) return {};
    const adapted = allTransactions.map(t => ({ id: t._key, cat: null, desc: t.desc, amt: t.amt, date: t.date }));
    const map = {};
    runAutoCatRules(adapted, rules).forEach(s => { map[s.txnId] = s; });
    return map;
  }, [allTransactions, rules]);

  const selectedRows = allTransactions.filter(t => !excluded.has(t._key));
  ...
  function toggleFile(fi)   {
    const fileKeys = allTransactions.filter(t => t._fileIdx === fi).map(t => t._key);
  ...
  // ── Review step ───────────────────────────────────────────────────────────
  const uniqueFiles = [...new Set(allTransactions.map(t => t._file))];
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:12 }}>
        {parsedFiles.map((pf, fi) => {
          const fileTxns    = allTransactions.filter(t => t._fileIdx === fi);
            ...
            >
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:11, color:'var(--stone)', marginTop:1 }}>
                  {pf.error
                    ? <span style={{ color:'var(--rd)' }}>⚠ {pf.error}</span>
      ...
      <div className="card">
        <div className="ch">
          ...
          <div style={{ marginLeft:'auto', display:'flex', gap:10, fontSize:12 }}>
            <span className="vp">↑ {selectedRows.filter(t=>t.amt>0).length} credits</span>
            <span className="vn">↓ {selectedRows.filter(t=>t.amt<0).length} debits</span>
            ...
            <table style={{ minWidth:720 }}>
              ...
              <tbody>
                ...
                {parsedFiles.map((pf, fi) => {
                  ...
                  const fileSelectedCnt = fileTxns.filter(t => !excluded.has(t._key)).length;
                  return (
              ...
              <tfoot>
                <tr style={{ background:'var(--sand)', borderTop:'0.5px solid var(--bd2)' }}>
                  ...
                  <td className="tr" style={{ padding:'8px 12px', fontSize:12 }}>
                    <div className="vp">{fmtAmt(selectedRows.filter(t=>t.amt>0).reduce((s,t)=>s+t.amt,0))}</div>
                    <div className="vn">{fmtAmt(selectedRows.filter(t=>t.amt<0).reduce((s,t)=>s+t.amt,0))}</div>
                  </td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
