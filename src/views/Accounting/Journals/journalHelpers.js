...
export function consolidateLines(lines, catMap) {
  const map = {};
  for (const l of lines) {
    ...
    if (!map[key]) map[key]={ key, account_name:name, code:cat?.code||null, type:cat?.t||cat?.type||null, parent_id:cat?.parent_id||null, cat_id:l.category_id||null, debit:0, credit:0 };
