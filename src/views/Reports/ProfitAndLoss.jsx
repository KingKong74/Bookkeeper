...
import { fmt, fmtSigned, filterByDateRange, buildPLFromJournals, dateRangeLabel } from '../../utils/helpers';
import { A4Paper, CompareBar, CompareHeader, StHead, StRow, StGroupTotal, StTotal, StGrand } from './reportComponents';
...
export function ProfitAndLoss() {
  ...
  function renderLines(lines,isExpense){if(!lines?.length)return<StRow label={`No ${isExpense?'expenses':'income'} in period`} value="" indent/>;const lineIds=new Set(lines.map(x=>x.id));const parents=lines.filter(x=>!x.parent_id||!lineIds.has(x.parent_id));const childrenOf=id=>lines.filter(x=>x.parent_id===id);return parents.flatMap(c=>{const kids=childrenOf(c.id),hasKids=kids.length>0;const getVal=item=>hasJournals?(isExpense?item.dr-item.cr:item.cr-item.dr):(isExpense?Math.abs(item.total):item.total);const val=fmt(getVal(c));const priorC=hasJournals?(isExpense?plP?.expenseLines:plP?.incomeLines)?.find(x=>x.id===c.id):byCatP[c.id];const valB=compare!=='none'?(priorC?fmt(getVal(priorC)):'—'):undefined;const vcls=isExpense?'vn':'vp';const kidsTotal=kids.reduce((s,ch)=>s+getVal(ch),0),ownVal=c.synthetic?0:getVal(c),groupTotal=hasKids?ownVal+kidsTotal:null;const groupTotalB=hasKids&&compare!=='none'?(()=>{const pKids=kids.reduce((s,ch)=>{const p=hasJournals?(isExpense?plP?.expenseLines:plP?.incomeLines)?.find(x=>x.id===ch.id):byCatP[ch.id];return s+(p?getVal(p):0);},0);const pOwn=c.synthetic?0:(priorC?getVal(priorC):0);return fmt(pOwn+pKids);})():undefined;const lbl=c.code?`${subCodeLabel(c.code,!!c.parent_id)||c.code}  ${c.l}`:c.l;return[hasKids?<StRow key={c.id} label={c.code?`${c.code}  ${c.l}`:c.l} value="" indent isParentHeader/>:<StRow key={c.id} label={lbl} value={val} valueB={valB} indent clickable onClick={()=>setDrill(c)} valueClass={vcls}/>,
    ...kids.map(ch=>{const cv=fmt(getVal(ch));const pch=hasJournals?(isExpense?plP?.expenseLines:plP?.incomeLines)?.find(x=>x.id===ch.id):byCatP[ch.id];const cvB=compare!=='none'?(pch?fmt(getVal(pch)):'—'):undefined;const sub=ch.code?.includes('/')?`/${ch.code.split('/')[1]}  ${ch.l}`:ch.l;return<StRow key={ch.id} label={sub} value={cv} valueB={cvB} sub clickable onClick={()=>setDrill(ch)} valueClass={vcls}/>;}),
    hasKids&&<StGroupTotal key={c.id+'-total'} label={c.l} value={fmt(groupTotal)} valueB={groupTotalB} valueClass={vcls}/>,].filter(Boolean);});}
  return(
    <div>
      <PeriodBar/>
