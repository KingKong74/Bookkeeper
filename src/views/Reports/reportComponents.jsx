...
// Monochrome mode context — when true, suppress red/green colouring on report lines
export const MonoContext = createContext(false);
export function useMono() { return useContext(MonoContext); }

export function A4Paper({ title, subtitle, children, wide=false }) {
  const { org } = useApp();
  const today = new Date().toLocaleDateString('en-AU', { day:'2-digit', month:'long', year:'numeric' });
  // Reactive dark mode — updates when data-theme changes without page reload
...
export function StRow({label,value,valueB,indent=false,sub=false,valueClass='',onClick,clickable=false,isParentHeader=false}){
  const leftPad = sub ? '60px' : indent ? '44px' : '32px';
  return (
    <div onClick={onClick} style={{ display:'flex', justifyContent:'space-between', padding:`5px 32px 5px ${leftPad}`, borderBottom:isParentHeader?'none':'0.5px solid var(--bd)', fontSize:12.5, cursor:clickable&&!isParentHeader?'pointer':'default', alignItems:'center' }} onMouseEnter={e=>{if(clickable&&!isParentHeader)e.currentTarget.style.background='var(--al)';}} onMouseLeave={e=>{e.currentTarget.style.background='';}}><span style={{ color:'var(--ink)', display:'flex', alignItems:'center', gap:5, fontWeight:isParentHeader?600:400 }}>{label}{clickable&&!isParentHeader&&<span style={{ fontSize:10, color:'var(--stone)', opacity:0.6 }}>↗</span>}</span><div style={{ display:'flex', gap:32, flexShrink:0 }}><span className={useMono()?'':valueClass} style={{ fontVariantNumeric:'tabular-nums', minWidth:90, textAlign:'right', color: useMono() ? 'var(--ink)' : undefined }}>{value}</span>{valueB!==undefined&&<span style={{ fontVariantNumeric:'tabular-nums', color:'var(--stone2)', minWidth:90, textAlign:'right', fontSize:12 }}>{valueB}</span>}</div>
    </div>
  );
}
...
export function BSRow({label,value,valueB,isNeg=false,onClick,clickable=false}){const mono=useMono();return(<div onClick={onClick} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'3px 32px', fontSize:12.5, cursor:clickable?'pointer':'default' }} onMouseEnter={e=>{if(clickable)e.currentTarget.style.background='var(--al)';}} onMouseLeave={e=>{e.currentTarget.style.background=''}}><span style={{ color:'var(--ink)', display:'flex', alignItems:'center', gap:5 }}>{label}{clickable&&<span style={{ fontSize:10, color:'var(--stone)', opacity:0.5 }}>↗</span>}</span><div style={{ display:'flex', gap:32 }}><span style={{ fontVariantNumeric:'tabular-nums', minWidth:110, textAlign:'right', color:mono?'var(--ink)':isNeg?'var(--rd)':'var(--a2)', fontSize:12.5 }}>{value}</span>{valueB!==undefined&&<span style={{ fontVariantNumeric:'tabular-nums', minWidth:110, textAlign:'right', color:'var(--stone2)', fontSize:12 }}>{valueB}</span>}</div></div>);}
